...
def test_features_site_ok(tmp_path: Path) -> None:
    ...
    args = argparse.Namespace(type="site", base=str(base), hazards=str(hazards), components=None, output=str(out), json=True, offline=False, cache_root=None)
...
def test_features_place_ok(tmp_path: Path) -> None:
    ...
    base.write_text("place_id,geo_level,geo_code,name,source_id\np-001,msa,12345,Test,connector.place_context\n")
    comps.write_text(
    ...
    args = argparse.Namespace(type="place", base=str(base), hazards=None, components=str(comps), output=str(out), json=True, offline=False, cache_root=None)
    rc = cli_main.cmd_features(args)
    assert rc == 0
    assert out.exists()
