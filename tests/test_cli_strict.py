...
def test_validate_ok_with_mocked_registry(monkeypatch: Any, capsys: Any) -> None:
...
def test_features_site_ok(tmp_path: Path) -> None:
    ...
    base.write_bytes(b"property_id,place_id,latitude,longitude\np1,pl1,39.7,-104.9\n")
    hazards.write_bytes(
        b"property_id,hazard_type,value,flag,as_of,source_id\n"
        b"p1,in_sfha,,True,2025-01-01,fema_nfhl\n"
    )
...
def test_features_place_ok(tmp_path: Path) -> None:
    ...
    base.write_bytes(
        b"place_id,geo_level,geo_code,name,source_id\np-001,msa,12345,Test,connector.place_context\n"
    )
    comps.write_bytes(
        b"amenities_15min_walk_count,avg_walk_time_to_top3_amenities,grocery_10min_drive_count," \
        b"intersection_density,public_land_acres_30min,minutes_to_trailhead,msa_jobs_t12,msa_jobs_t36," \
        b"slope_gt15_pct_within_10km,protected_land_within_10km_pct,permits_5plus_per_1k_hh_t12\n" \
        b"10,15.0,5,100,1000,20,1.5,4.0,5.0,20.0,2.0\n"
    )
    args = argparse.Namespace(
        type="place",
        base=str(base),
